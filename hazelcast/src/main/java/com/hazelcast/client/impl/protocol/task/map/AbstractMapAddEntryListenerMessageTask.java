...
public abstract class AbstractMapAddEntryListenerMessageTask<Parameter>
    ...
    protected CompletableFuture<EventRegistration> processInternal() {
        ...
        if (isLocalOnly()) {
            eventRegistration = mapServiceContext.addLocalEventListener(listener, eventFilter, name);
        } else {
            eventRegistration = mapServiceContext.addEventListener(listener, eventFilter, name);
        }
